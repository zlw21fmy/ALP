...
Gain_clean_series_2 = repmat(Gain_clean,num_2,1); %20个干净的谱 纵向串联
 
for j= 1:num_1
Gain_noisy_series_2(:,j) = awgn(Gain_clean_series_2(:,j),snr_given,'measured');
end
%% 
% M_matrix = [5;6;7;8;9;10;15;20;25;30;35;40;45;50;55;60;65;70]';
...
for jj = 1:length(M_matrix)
    ...
    for kk = 1:length(mu_matrix)
       ...
       %%  按第2种计算方式  最小误差为 0.239 MHz
       BFS_filtered_error_3(ii,:) = abs((mean(BFS_filtered(:,3:end)'))'-(vb));
	   BFS_filtered_error_3_mean = mean(BFS_filtered_error_3(ii,:));
       ...
       disp([  '第三种衡量方式下  滤波后频移结果为' num2str(BFS_filtered_error_3_mean)  ]);
        end
    BFS_error_average_1(jj,kk) = mean(mean(BFS_filtered_error_1));
   
    BFS_error_average_3(jj,kk) = mean(mean(BFS_filtered_error_3));
